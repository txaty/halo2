...
pub(crate) struct PermutationProof<P: Pairing> {
    g1_adjusted_permutation: P::G1,
    g1_quotient: P::G1,
}
...
pub(crate) fn create_permutation_proof<P: Pairing>(
    ...
    let g1_adjusted_permutation =
        Kzg::<<P as Pairing>::G1>::commit(g1_affine_srs, &poly_adjusted_permutation);
    let g1_quotient = Kzg::<<P as Pairing>::G1>::commit(g1_affine_srs, &poly_quotient);

    PermutationProof {
        g1_adjusted_permutation,
        g1_quotient,
    }
}
...
pub(crate) fn verify_permutation_proof<P: Pairing>(
    ...
    adjusted_permutation_statement: &P::G1Affine,
    g1_affine_permutation_padding: &P::G1Affine,
    permutation_proof: &PermutationProof<P>,
    ...
    let permutation_subtracted = adjusted_permutation_statement
        .into_group()
        .sub(g1_affine_permutation_padding);

    let right_pairing = P::multi_pairing(
        &[permutation_proof.g1_quotient, permutation_subtracted],
...
mod tests {
    ...
    fn test_permutation_proof() {
        ...
        let poly_permutation = DensePolynomial::from_coefficients_vec(
            domain.ifft(&permutation_witness_values),
        );
        let permutation_statement = Kzg::<G1>::commit(&powers_of_tau_g1, &poly_permutation)
            .into_affine();
        let poly_adjusted_permutation = DensePolynomial::from_coefficients_vec(
            domain.ifft(&adjusted_permutation_values),
        );
        let adjusted_permutation_statement = Kzg::<G1>::commit(&powers_of_tau_g1, &poly_adjusted_permutation)
            .into_affine();
        let g1_affine_permutation_padding = Kzg::<G1>::commit(&powers_of_tau_g1, &poly_permutation_padding)
            .into_affine();
        let vanishing_poly  = domain.vanishing_polynomial().into();
        let g2_vanishing_poly_v = Kzg::<G2>::commit(&powers_of_tau_g2, &vanishing_poly).into_affine();
