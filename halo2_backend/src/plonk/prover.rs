...
#[derive(Debug)]
pub(super) struct InstanceSingle<C: CurveAffine> {
...
#[derive(Debug, Clone)]
pub(super) struct AdviceSingle<C: CurveAffine, B: Basis> {
...
pub struct Prover<
    ...
    M: MsmAccel<Scheme::Curve>,
> {
    pub(super) engine: PlonkEngine<Scheme::Curve, M>,
    // Circuit and setup fields
    pub(super) params: &'params Scheme::ParamsProver,
    // Plonk proving key
    pub(super) pk: &'a ProvingKey<Scheme::Curve>,
    // Phases
    pub(super)  phases: Vec<u8>,
    // Polynomials (Lagrange and Coeff) for all circuits instances
    pub(super) instances: Vec<InstanceSingle<Scheme::Curve>>,
    // Advice polynomials with its blindings
    pub(super) advices: Vec<AdviceSingle<Scheme::Curve, LagrangeCoeff>>,
    // The phase challenges by challenge index
    pub(super) challenges: HashMap<usize, Scheme::Scalar>,
    // The next phase to be committed
    pub(super) next_phase_index: usize,
    // Transcript to be updated
    pub(super) transcript: &'a mut T,
    // Randomness
    pub(super) rng: R,
    pub(super) _marker: std::marker::PhantomData<(P, E)>,
}
